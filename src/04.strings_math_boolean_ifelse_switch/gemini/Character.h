...
약점: 발바닥 만지기 (간지럼을 못 참음), 다이어트 강요*/
        ...
        {
            /*알겠습니다. 요청하신 대로 C++ 스터디를 위한 캐릭터 디자인과 생성에 집중하겠습니다.
    ...
    기존 캐릭터 상세 정보 생성
    다음은 initializeCharacters() 함수에 정의되어 있지만, 상세 정보가 부족했던 세 캐릭터에 대한 정보입니다.
    ...
    나이: 23세 (C++ 스터디에 이제 막 흥미를 붙인 초등학생)

    HP: 10

    Power: 1

    재산: 5조5억원 (대한민국 평정한 재벌, NY까지 진출 가능)
    ...
    표정 (추가될 내용 - display 함수 내 else 블록에 포함):
...
std::vector<Character> initializeCharacters()
{
    ...
    characters.push_back(Character("보리", "Bori", 50, 50));
    ...
    characters.push_back(Character("임정찬바보", "Liki", 100, 1));
