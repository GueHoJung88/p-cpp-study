...
#include <sstream>  // std::stringstream을 사용하기 위해 추가
...
// 모든 쪽지 시험 퀴즈 데이터를 생성하고 반환하는 함수 (총 60문제 예시)
vector<Question> initializePopQuizQuestions() {
    return {
        // --- 1. Syntax (문법) 관련 퀴즈 (5개) ---
        {"Syntax", "C++ 코드의 각 문장 끝에 반드시 붙여야 하는 기호는 무엇일까요?", "문장의 끝을 나타내는 마침표와 같아요.",
            {{"점(.)", "주로 객체 멤버 접근에 사용돼요.", false}, {"세미콜론(;)", "정답! C++에서는 각 문장(statement)의 끝에 세미콜론을 붙여야 합니다.", true}, {"콜론(:)", "주로 레이블이나 초기화 리스트에 사용돼요.", false}, {"쉼표(,)", "주로 변수 선언 구분이나 함수 인자 구분해요.", false}}
        },
        {"Syntax", "C++ 프로그램의 실행이 시작되는 특별한 함수는 무엇일까요?", "모든 프로그램의 '입구' 역할을 하는 함수예요.",
            {{"start()", "표준 함수 이름이 아니에요.", false}, {"main()", "정답! C++ 프로그램은 `main()` 함수부터 실행됩니다.", true}, {"run()", "표준 함수 이름이 아니에요.", false}, {"execute()", "표준 함수 이름이 아니에요.", false}}
        },
        {"Syntax", "C++에서 코드 블록(여러 문장 묶음)을 정의할 때 사용하는 기호는 무엇일까요?", "여러 문장을 한 묶음으로 만들 때 쓰는 괄호예요.",
            {{"소괄호 ()", "함수 호출이나 우선순위 지정에 사용돼요.", false}, {"대괄호 []", "배열 접근이나 속성 접근에 사용돼요.", false}, {"중괄호 {}", "정답! 중괄호는 함수 본문, 조건문, 반복문 등 코드 블록을 정의하는 데 사용됩니다.", true}, {"꺾쇠괄호 <>", "템플릿이나 헤더 파일 포함에 사용돼요.", false}}
        },
        {"Syntax", "C++에서 변수를 선언할 때, 변수 이름 앞에 반드시 붙여야 하는 것은 무엇일까요?", "데이터를 어떤 형태로 저장할지 알려주는 것이에요.",
            {{"값", "값은 변수 뒤에 할당돼요.", false}, {"세미콜론", "문장 끝에 붙어요.", false}, {"데이터 타입", "정답! 변수 이름 앞에 해당 변수가 어떤 종류의 데이터를 저장할지 나타내는 데이터 타입을 명시해야 합니다.", true}, {"콜론", "변수 선언에 사용되지 않아요.", false}}
        },
        {"Syntax", "C++에서 미리 정의된 기능들을 사용하기 위해 코드 상단에 포함하는 것은 무엇일까요? (예: `<iostream>`)", "프로그램에 필요한 '도구 상자'를 가져오는 행위예요.",
            {{"주석", "코드를 설명하는 부분이에요.", false}, {"변수 선언", "변수를 만드는 부분이에요.", false}, {"헤더 파일", "정답! `<iostream>`과 같은 헤더 파일은 표준 라이브러리의 기능(함수, 클래스 등)을 사용하기 위해 프로그램에 포함됩니다.", true}, {"함수 호출", "이미 정의된 함수를 사용하는 행위예요.", false}}
        },

        // --- 2. Output (출력) 관련 퀴즈 (5개) ---
        {"Output", "C++에서 화면에 텍스트나 변수 값을 출력할 때 사용하는 기본 객체는 무엇일까요?", "콘솔에 '내보내는' 역할을 해요.",
            {{"cin", "입력 객체예요.", false}, {"cout", "정답! `cout` 객체와 `<<` 연산자를 사용하여 콘솔에 데이터를 출력합니다.", true}, {"print", "C++ 표준 객체가 아니에요. 다른 언어에서 사용돼요.", false}, {"output", "C++ 표준 객체가 아니에요.", false}}
        },
        {"Output", "`cout << \"Hello\";` 코드는 화면에 무엇을 출력할까요?", "큰따옴표 안의 내용을 그대로 출력해요.",
            {{"Hello", "정답! 큰따옴표로 감싼 문자열은 `cout`에 의해 그대로 화면에 출력됩니다.", true}, {"\"Hello\"", "큰따옴표는 출력되지 않아요.", false}, {"에러", "유효한 출력 구문이에요.", false}, {"아무것도 출력되지 않음", "출력될 내용이 명확해요.", false}}
        },
        {"Output", "출력 스트림에서 다음 출력을 새로운 줄로 이동시키는 데 주로 사용하는 것은 무엇일까요?", "줄바꿈을 의미하는 제어 문자 또는 조작자예요.",
            {{"\\t", "탭 문자를 출력해요.", false}, {"space", "공백을 출력해요.", false}, {"endl", "정답! `endl` (end line)은 새로운 줄로 이동하고 출력 버퍼를 비우는 역할을 합니다.", true}, {"newline()", "C++ 표준 함수가 아니에요.", false}}
        },
        {"Output", "`int num = 123; cout << \"Number: \" << num;` 코드는 화면에 무엇을 출력할까요?", "문자열과 변수 값을 연결해서 출력해요.",
            {{"Number: num", "변수 이름이 아니라 값이 출력돼요.", false}, {"123", "문자열도 함께 출력돼요.", false}, {"Number: 123", "정답! 문자열 리터럴과 변수의 값이 연결되어 출력됩니다.", true}, {"에러", "유효한 출력 구문이에요.", false}}
        },
        {"Output", "다음 중 C++에서 출력할 때 오류가 발생하는 경우는 무엇일까요?", "올바른 출력 구문이 아닌 것을 찾아보세요.",
            {{"cout << 100;", "숫자 출력은 가능해요.", false}, {"cout << \"Text\";", "문자열 출력은 가능해요.", false}, {"cout << (10 + 5);", "표현식의 결과도 출력 가능해요.", false}, {"cout >> \"Hello\";", "정답! `cout`은 출력 객체이므로 `>>` (추출) 연산자와 함께 사용할 수 없습니다. `<<` (삽입) 연산자를 사용해야 합니다.", true}}
        },
        
        // --- 3. Comments (주석) 관련 퀴즈 (5개) ---
        {"Comments", "C++에서 한 줄 주석을 작성할 때 사용하는 기호는 무엇일까요?", "한 줄만 설명을 달 때 쓰는 기호예요.",
            {{"/* ... */", "여러 줄 주석에 사용돼요.", false}, {"//", "정답! `//` 뒤에 오는 내용은 한 줄 주석으로 처리됩니다.", true}, {"#", "매크로 정의나 전처리기 지시문에 사용돼요. 다른 언어의 주석 기호이기도 해요.", false}, {"", "HTML 주석에 사용돼요.", false}}
        },
        {"Comments", "C++에서 여러 줄 주석을 시작하고 끝낼 때 사용하는 기호는 무엇일까요?", "블록 단위로 설명을 달 때 쓰는 기호예요.",
            {{"//", "한 줄 주석에 사용돼요.", false}, {"/* ... */", "정답! `/*`로 시작하고 `*/`로 끝나는 블록은 여러 줄 주석으로 처리됩니다.", true}, {"<-- ... -->", "HTML 주석에 사용돼요.", false}, {"###", "다른 언어의 주석 기호예요.", false}}
        },
        {"Comments", "주석 처리된 코드가 프로그램 실행에 미치는 영향은 무엇일까요?", "컴파일러가 이 부분을 어떻게 처리하는지 생각해보세요.",
            {{"오류를 발생시킨다.", "주석은 오류를 발생시키지 않아요.", false}, {"실행 속도를 느리게 한다.", "컴파일러가 주석을 무시하므로 실행 속도에 영향 없어요.", false}, {"컴파일러가 해당 부분을 무시하고 실행에 포함시키지 않는다.", "정답! 주석은 컴파일 과정에서 무시되므로, 프로그램 실행에는 전혀 영향을 미치지 않습니다.", true}, {"특정 조건에서만 실행된다.", "주석은 실행되는 코드가 아니에요.", false}}
        },
        {"Comments", "주석을 사용하는 주된 목적은 무엇일까요?", "코드의 이해도를 높이는 목적을 생각해보세요.",
            {{"프로그램 크기를 줄이기 위함", "주석은 컴파일 후 실행 파일에 포함되지 않으므로, 프로그램 크기와 직접 관련 없어요.", false}, {"코드를 더 빠르게 실행하기 위함", "실행 속도에 영향 없어요.", false}, {"코드의 가독성을 높이고 다른 개발자(또는 미래의 자신)에게 코드 설명을 제공하기 위함", "정답! 주석은 코드의 기능을 설명하고, 복잡한 로직을 이해하는 데 도움을 줍니다.", true}, {"변수 이름을 정의하기 위함", "변수 이름은 따로 정의해요.", false}}
        },
        {"Comments", "C++에서 주석 안에 다른 주석을 중첩하여 사용할 수 있는가? (예: `/* // 한 줄 주석 */`)", "주석 문법의 중첩 가능성을 생각해보세요.",
            {{"가능하다, 모든 주석은 중첩될 수 있다.", "모든 주석이 중첩되는 것은 아니에요.", false}, {"가능하지만, 한 줄 주석(`//`)만 여러 줄 주석(`/* ... */`) 안에 중첩될 수 있다.", "정답! `/* ... */` 주석 안에 `//` 주석은 가능하지만, `/* ... */` 주석 안에 또 다른 `/* ... */` 주석을 중첩하는 것은 일반적으로 컴파일 에러를 발생시킵니다.", true}, {"불가능하다, 항상 에러가 발생한다.", "일부 경우는 가능해요.", false}, {"특정 컴파일러에서만 가능하다.", "대부분의 표준 컴파일러에서 동일해요.", false}}
        },

        // --- 4. Variables (변수) 관련 퀴즈 (확장, 총 15개) ---
        ...
        {"Variables", "변수 `int price = 1000;`에 `+= 500;` 연산을 적용했을 때, `price`의 최종 값은 얼마가 될까요?", "기존 값에 오른쪽 값을 더해서 다시 저장하는 축약 연산자예요.",
            {{"500", "이것은 덧셈 연산일 뿐 값을 바꾸지 않아요.", false}, {"1000", "값이 변경되지 않아요.", false}, {"1500", "`+=` 연산은 기존 변수 값에 오른쪽 값을 더한 후 다시 변수에 저장하므로, 1000 + 500 = 1500이 됩니다.", true}, {"컴파일 에러", "유효한 복합 할당 연산자입니다.", false}}
        },
        {"Variables", "다음 중 C++에서 `double` 타입과 `float` 타입의 주요 차이점은 무엇일까요?", "소수점 숫자의 '정밀도'와 '크기'에 대한 차이를 생각해보세요.",
            {{"`double`은 정수만 저장하고, `float`은 소수점만 저장한다.", "둘 다 소수점을 저장해요.", false}, {"`double`은 `float`보다 더 큰 범위와 높은 정밀도를 가진다.", "`double`이 더 많은 메모리를 사용하고 더 정밀해요.", true}, {"`float`는 `double`보다 더 많은 메모리를 차지한다.", "`double`이 더 많아요.", false}, {"두 타입 모두 완전히 동일하며 차이가 없다.", "차이가 있어요.", false}}
        },
        {"Variables", "`int x = 5, y = 2;`일 때, `x / y`의 결과는 무엇일까요?", "정수와 정수의 나눗셈은 결과도 정수라는 점을 기억하세요.",
            {{"2.5", "소수점 이하를 버려요.", false}, {"2", "정수 나눗셈은 소수점 이하를 버리고 몫만 반환합니다.", true}, {"3", "반올림하지 않아요.", false}, {"컴파일 에러", "유효한 연산이에요.", false}}
        },

        // --- 5. Operators (연산자) 관련 퀴즈 (5개) ---
        {"Operators", "`int a = 10; int b = 3; int remainder = a % b;`에서 `remainder`의 값은 무엇일까요?", "나머지 연산자(모듈러스)를 떠올려보세요.",
            {{"0", "나누어 떨어지지 않아요.", false}, {"1", "정답! 10을 3으로 나눈 나머지는 1입니다.", true}, {"3", "몫이에요.", false}, {"10", "피연산자가 아니에요.", false}}
        },
        {"Operators", "`int x = 5; x++;` 코드가 실행된 후, `x`의 값은 얼마가 될까요?", "증가 연산자(`++`)의 역할을 생각해보세요.",
            {{"4", "감소 연산자(--)에 해당해요.", false}, {"5", "값이 변해요.", false}, {"6", "정답! `x++`는 `x`의 값을 1 증가시킵니다.", true}, {"컴파일 에러", "유효한 연산이에요.", false}}
        },
        {"Operators", "`int a = 7; int b = 4; bool result = (a > b) && (a % 2 != 0);`에서 `result`의 값은 무엇일까요?", "비교 및 논리 연산자를 조합해서 판단하세요.",
            {{"true", "`a > b`는 참(7 > 4)이고, `a % 2 != 0`도 참(7을 2로 나눈 나머지는 1이고 0이 아님)이므로, `참 && 참`은 참이 됩니다.", true}, {"false", "두 조건 모두 참이에요.", false}, {"컴파일 에러", "유효한 코드예요.", false}, {"7", "결과는 불리언 값이에요.", false}}
        },
        {"Operators", "다음 중 '같지 않다'를 의미하는 비교 연산자는 무엇일까요?", "같다(`==`)와 반대되는 기호를 찾아보세요.",
            {{"=", "할당 연산자예요.", false}, {"==", "같다 연산자예요.", false}, {"!=", "정답! `!=`는 두 값이 같지 않을 때 참을 반환합니다.", true}, {"<>", "일부 언어에서 사용되지만 C++ 표준 연산자가 아니에요.", false}}
        },
        {"Operators", "`int x = 10; x *= 2;` 코드가 실행된 후, `x`의 값은 얼마가 될까요?", "복합 할당 연산자(`*=`)를 떠올려보세요.",
            {{"10", "값이 변해요.", false}, {"12", "덧셈이 아니에요.", false}, {"20", "정답! `x *= 2`는 `x = x * 2`와 같으므로, 10 * 2 = 20이 됩니다.", true}, {"컴파일 에러", "유효한 연산이에요.", false}}
        },

        // --- 6. Data Types (데이터 타입) 관련 퀴즈 (5개) ---
        {"Data Types", "문자 1개를 저장하는 데 가장 적합한 기본 데이터 타입은 무엇일까요?", "한 글자만 담는 그릇이에요.",
            {{"string", "문자열을 저장해요.", false}, {"char", "정답! `char`는 단일 문자(character)를 저장하는 데 사용되는 기본 데이터 타입입니다.", true}, {"int", "정수를 저장해요.", false}, {"double", "실수를 저장해요.", false}}
        },
        {"Data Types", "소수점 이하가 있는 숫자(예: 3.14)를 저장하는 데 가장 적합한 기본 데이터 타입은 무엇일까요?", "정밀한 실수를 담는 그릇이에요.",
            {{"int", "정수를 저장해요.", false}, {"float", "정답! `float`와 `double`은 소수점이 있는 실수를 저장하는 데 사용됩니다. `float`는 단정밀도 부동소수점입니다.", true}, {"char", "단일 문자를 저장해요.", false}, {"bool", "참/거짓을 저장해요.", false}}
        },
        {"Data Types", "`true` 또는 `false` 논리 값을 저장하는 데 사용하는 데이터 타입은 무엇일까요?", "참 거짓을 담는 그릇이에요.",
            {{"int", "정수를 저장해요. 0과 1로 표현되지만 bool이 더 적합해요.", false}, {"string", "문자열을 저장해요.", false}, {"bool", "정답! `bool`은 불리언(Boolean) 값을 저장하는 데 사용되는 타입입니다.", true}, {"char", "단일 문자를 저장해요.", false}}
        },
        {"Data Types", "C++에서 정수를 저장하는 기본 데이터 타입인 `int`는 일반적으로 몇 비트(bit) 크기를 가질까요?", "컴퓨터 시스템의 일반적인 크기를 생각해보세요.",
            {{"8 비트", "char의 일반적인 크기예요.", false}, {"16 비트", "short int의 일반적인 크기예요.", false}, {"32 비트", "정답! 대부분의 현대 시스템에서 `int`는 32비트(4바이트) 크기를 가집니다.", true}, {"64 비트", "long long int의 일반적인 크기예요.", false}}
        },
        {"Data Types", "다음 C++ 코드에서 `myFloat`의 데이터 타입으로 가장 적절한 것은?\n`float myFloat = 10.5f;`", "선언 키워드와 숫자 뒤에 붙은 `f`를 확인하세요.",
            {{"int", "`f` 접미사는 부동소수점이에요.", false}, {"double", "`f` 접미사는 `float`을 나타내요. `double`은 접미사가 없거나 `L` 접미사를 사용해요.", false}, {"float", "정답! `float` 키워드로 선언되었고 `f` 접미사는 리터럴이 `float` 타입임을 명시합니다.", true}, {"long", "정수가 아니에요.", false}}
        },

        // --- 7. User Input (사용자 입력) 관련 퀴즈 (5개) ---
        {"User Input", "사용자로부터 키보드 입력을 받아 변수에 저장할 때 사용하는 C++의 기본 객체는 무엇일까요?", "콘솔로부터 '받아들이는' 역할을 해요.",
            {{"cout", "출력 객체예요.", false}, {"cin", "정답! `cin` 객체와 `>>` 연산자를 사용하여 사용자로부터 입력을 받습니다.", true}, {"scanf", "C-스타일 입력 함수예요.", false}, {"input", "다른 언어에서 사용돼요.", false}}
        },
        {"User Input", "`cin >> myVariable;`에서 `>>` 연산자를 무엇이라고 부를까요?", "입력 스트림에서 값을 '추출'하는 연산자예요.",
            {{"삽입 연산자", "출력(`<<`)에 사용돼요.", false}, {"추출 연산자", "정답! `>>`는 입력 스트림에서 데이터를 추출하여 변수에 저장하는 연산자입니다.", true}, {"할당 연산자", "값 대입(`=`)에 사용돼요.", false}, {"비교 연산자", "두 값을 비교해요.", false}}
        },
        {"User Input", "사용자가 공백이 포함된 문장(예: \"Hello World\")을 입력했을 때, `cin >> myString;`은 어떤 문제가 발생할 수 있나요?", "공백이 입력의 끝으로 간주될 수 있어요.",
            {{"입력 버퍼가 넘친다.", "버퍼 오버플로우와는 달라요.", false}, {"첫 번째 단어만 `myString`에 저장된다.", "정답! `cin >>`는 공백을 구분자로 인식하여 첫 번째 단어만 읽어들이고, 나머지 단어는 입력 버퍼에 남깁니다.", true}, {"컴파일 에러가 발생한다.", "유효한 구문이에요.", false}, {"아무것도 저장되지 않는다.", "첫 단어는 저장돼요.", false}}
        },
        {"User Input", "공백이 포함된 한 줄 전체의 문자열을 사용자로부터 입력받으려면 어떤 함수를 사용해야 할까요?", "한 '줄'을 통째로 읽어들이는 함수예요.",
            {{"cin.getLine()", "정확한 멤버 함수 이름이 아니에요.", false}, {"std::getline(std::cin, myString);", "정답! `std::getline()` 함수는 공백을 포함한 한 줄 전체의 텍스트를 읽어 `std::string` 변수에 저장합니다.", true}, {"std::cin.read();", "지정된 바이트만큼 읽어요.", false}, {"inputString()", "C++ 표준 함수가 아니에요.", false}}
        },
        {"User Input", "`cin >> intValue;`를 사용한 후, 바로 `std::getline(std::cin, stringValue);`를 사용하면 발생할 수 있는 흔한 문제점은 무엇이며, 어떻게 해결해야 할까요?", "입력 버퍼에 남은 줄바꿈 문자를 처리하는 방법을 생각해보세요.",
            {{"컴파일 에러 발생; 코드를 다시 작성해야 한다.", "에러가 아니라 런타임 동작 문제예요.", false}, {"`getline`이 빈 줄을 읽어버림; `cin.ignore()`로 버퍼를 비워야 한다.", "정답! `cin >>`는 숫자 입력 후 남은 줄바꿈 문자를 입력 버퍼에 남기는데, `getline()`이 이를 바로 읽어버려 빈 문자열을 저장합니다. `cin.ignore()`로 이 줄바꿈 문자를 제거해야 합니다.", true}, {"`getline`이 두 줄을 읽어버림; `getline`을 두 번 호출해야 한다.", "두 줄을 읽는 게 아니에요.", false}, {"문제 없음; C++이 자동으로 처리한다.", "자동으로 처리되지 않아요.", false}}
        },

        // --- 8. Strings (문자열) 관련 퀴즈 (총 20개) ---
        ...
        {"Strings", "공백을 포함한 한 줄 전체의 텍스트를 사용자로부터 입력받기 위해 사용하는 C++ 함수는 무엇일까요?", "입출력 객체 `cin`과 함께 사용되며, 한 '라인'을 읽어들여요.",
            {{"read()", "read()는 다른 용도로 사용되는 함수입니다.", false}, {"input()", "input()은 C++ 표준 라이브러리의 함수가 아니에요.", false}, {"getline(cin, myString);", "정답! `std::getline()` 함수는 공백을 포함한 한 줄 전체의 텍스트를 읽어 `std::string` 변수에 저장합니다.", true}, {"cin.readLine();", "readLine()은 C++ 표준 cin 객체에 존재하지 않습니다.", false}}
        },
        ...
        {"Math", "`double value = 5.9;`를 내림(floor)하여 정수로 만들려면 `cmath` 라이브러리의 어떤 함수를 사용해야 할까요?", "바닥(floor)을 의미하는 영어 단어를 떠올려보세요.",
            {{"round()", "round()는 반올림이에요.", false}, {"ceil()", "ceil()은 올림이에요.", false}, {"floor()", "정답! floor() 함수는 주어진 숫자보다 작거나 같은 가장 큰 정수를 반환합니다 (내림).", true}, {"trunc()", "trunc()는 소수점 이하를 버릴 때 사용해요 (양수는 floor와 같고, 음수는 ceil과 같아요).", false}}
        },
...
void pressEnterToContinue(const Character& speaker, const string& emotion, const string& message) {
    ...
    cout << speaker.name << ": " << message << endl;
...
// 쪽지 시험 퀴즈 진행 함수
void conductPopQuiz(Character& player, const vector<Question>& all_questions_bank, const std::map<int, string>& categories_map) {
    std::map<string, int> category_correct_counts;
    std::map<string, int> category_total_counts;

    // 모든 카테고리 초기화
    for (const auto& pair : categories_map) {
        category_correct_counts[pair.second] = 0;
        category_total_counts[pair.second] = 0;
    }

    bool continue_selection = true;
    while (continue_selection) {
        cout << "\n========== 쪽지 시험 카테고리 선택 ==========" << endl;
        player.display("thinking");
        cout << player.name << ": 어떤 부분에 대해 쪽지 시험을 보고 싶어? 여러 개 선택할 수 있어! (번호를 띄어쓰기로 구분해서 입력해줘)" << endl;
        cout << player.name << ": 선택을 마쳤으면 엔터를 눌러줘! 멍멍!" << endl;
        
        for (const auto& pair : categories_map) {
            cout << pair.first << ". " << pair.second << endl;
        }
        cout << "예시) 1 2 5 (Syntax, Output, If-Else 선택)" << endl;
        
        string line_input;
        std::getline(cin, line_input);
        std::stringstream ss(line_input);
        int category_choice_num;
        
        vector<string> selected_categories;
        while (ss >> category_choice_num) {
            if (categories_map.count(category_choice_num)) {
                selected_categories.push_back(categories_map.at(category_choice_num));
            } else {
                cout << player.name << ": 멍? " << category_choice_num << "번은 없는 카테고리인 것 같아... 다시 확인해줘!" << endl;
                player.display("sad");
            }
        }

        if (selected_categories.empty()) {
            cout << player.name << ": 으음... 아무것도 선택 안 했어? 다시 선택해줘!" << endl;
            player.display("sad");
            continue; // 다시 선택 루프로
        }

        cout << player.name << ": 좋아! 선택한 카테고리들로 쪽지 시험을 시작한다멍!" << endl;
        player.display("happy");
        pressEnterToContinue(player, "happy", "쪽지 시험 시작!");

        // 선택된 카테고리에서 문제 뽑기
        vector<Question> quiz_for_this_session;
        for (const string& cat_name : selected_categories) {
            vector<Question> questions_in_category;
            for (const auto& q : all_questions_bank) {
                if (q.category == cat_name) {
                    questions_in_category.push_back(q);
                }
            }
            std::random_shuffle(questions_in_category.begin(), questions_in_category.end());

            // 각 카테고리에서 3~5문제 랜덤 선택
            int num_to_pick = std::min((int)questions_in_category.size(), 3 + (rand() % 3)); // 3, 4, 5 중 랜덤
            for (int i = 0; i < num_to_pick; ++i) {
                quiz_for_this_session.push_back(questions_in_category[i]);
            }
        }
        std::random_shuffle(quiz_for_this_session.begin(), quiz_for_this_session.end()); // 최종 문제들 다시 섞기

        int correct_answers_session = 0;
        int total_questions_session = quiz_for_this_session.size();

        if (total_questions_session == 0) {
            cout << player.name << ": 앗! 선택한 카테고리에 아직 문제가 없어! 다른 카테고리를 선택해줘! 멍멍!" << endl;
            player.display("sad");
            pressEnterToContinue(player, "sad", "문제 추가를 기다린다멍!");
            continue;
        }

        Character pop_quiz_master = initializeCharacters()[rand() % initializeCharacters().size()];
        // 플레이어 캐릭터와 겹치지 않게
        while (pop_quiz_master.name == player.name && pop_quiz_master.species == player.species) {
            pop_quiz_master = initializeCharacters()[rand() % initializeCharacters().size()];
        }
        vector<string> pop_quiz_master_nicknames = {"꼬마 교수님", "지식 요정", "문제 탐정", "똑똑이 친구"};
        pop_quiz_master.name = pop_quiz_master_nicknames[rand() % pop_quiz_master_nicknames.size()] + " " + pop_quiz_master.species;


        for (int i = 0; i < total_questions_session; ++i) {
            if (player.hp <= 0) {
                cout << "\n" << player.name << ": 으윽... 체력이... 더 이상은... (털썩)" << endl;
                player.display("sad");
                cout << pop_quiz_master.name << ": 아쉽군... " << player.name << "가 기절하고 말았다냥! 다시 체력을 회복하고 도전하라냥!" << endl;
                pop_quiz_master.display("sad");
                pressEnterToContinue(pop_quiz_master, "sad", "쪽지 시험은 여기까지다냥...");
                return; // 퀴즈 종료
            }

            Question q = quiz_for_this_session[i];
            
            cout << "\n========== 쪽지 시험 문제 " << i + 1 << " (" << q.category << ") ==========" << endl;
            pop_quiz_master.display("thinking");
            cout << pop_quiz_master.name << ": " << player.name << ", 이 문제에 집중해봐라냥!" << endl;
            ...
            while (true) {
                ...
                if (cin.fail() || user_choice <= 0 || user_choice > current_options.size()) {
                    ...
                    pop_quiz_master.display("angry");
                    cout << pop_quiz_master.name << ": " << player.name << ", 똑바로 입력해라냥! 허튼짓 하면 혼난다냥!" << endl;
                } else {
                    break;
                }
            }
            ...
            category_total_counts[q.category]++; // 선택된 카테고리의 총 문제 수 증가

            cout << "\n--- 결과 ---" << endl;
            if (chosen_answer.isCorrect) {
                correct_answers_session++;
                category_correct_counts[q.category]++; // 선택된 카테고리의 정답 수 증가
                player.display("happy");
                cout << player.name << ": 야호! 정답이다! 나 천재인가봐! 고구마 냠냠!" << endl;
                pop_quiz_master.display("surprised");
                cout << pop_quiz_master.name << ": 훌륭하다냥! 정답이다냥! 네 지식이 대단하다냥!" << endl;
                cout << "[설명] " << chosen_answer.rationale << endl;
                player.hp += 10; // 정답 시 HP 회복
                cout << player.name << "의 체력이 10 회복되었어! (현재 HP: " << player.hp << ")" << endl;
            } else {
                player.display("sad");
                cout << player.name << ": 으악! 틀렸어... 멍... 머리가 지끈거려... (고구마 줘)" << endl;
                pop_quiz_master.display("angry");
                cout << pop_quiz_master.name << ": 쯧쯧! 틀렸다냥! " << player.name << ", 다시 공부해야 한다냥!" << endl;
                
                // 패널티 요소: 문제 낸 친구가 실망해서 떠나고 다른 친구가 등장
                if (rand() % 2 == 0) { // 50% 확률로 친구가 떠남
                    cout << pop_quiz_master.name << ": 이 문제도 못 맞추다니... 실망이다냥! 나는 간다냥! (휙 사라짐)" << endl;
                    pop_quiz_master.display("sad");
                    // 새로운 캐릭터 등장
                    Character new_friend = initializeCharacters()[rand() % initializeCharacters().size()];
                    while ((new_friend.name == player.name && new_friend.species == player.species) || (new_friend.name == pop_quiz_master.name && new_friend.species == pop_quiz_master.species)) {
                        new_friend = initializeCharacters()[rand() % initializeCharacters().size()];
                    }
                    cout << new_friend.name << " (" << new_friend.species << "): " << player.name << "! 힘내! 내가 옆에서 응원해줄게! 이 문제 풀이는 내가 도와줄게!" << endl;
                    new_friend.display("happy");
                    pop_quiz_master = new_friend; // 다음 문제부터 이 친구가 출제
                } else {
                     cout << pop_quiz_master.name << ": 벌칙이다냥! " << player.name << ", 앞구르기 3번이다냥! (냥냥펀치)" << endl;
                     player.display("thinking"); // 앞구르기 시도
                     cout << player.name << ": 멍... (어설프게 앞구르기)... 삐끗..." << endl;
                }
                string correct_answer_text = "정답을 찾을 수 없습니다.";
                ...
                cout << "[정답 풀이] 정답은 '" << correct_answer_text << "' 야! " << chosen_answer.rationale << endl;
        ...
        cout << "\n\n========== 쪽지 시험 결과 ==========" << endl;
        player.display("happy");
        cout << player.name << ": 휴... 쪽지 시험이 끝났다! 결과는 어떨까?" << endl;
        pop_quiz_master.display("default");
        cout << pop_quiz_master.name << ": 자, " << player.name << "! 네 쪽지 시험 결과다냥!" << endl;

        cout << "총 문제 수: " << total_questions_session << endl;
        cout << "맞춘 문제 수: " << correct_answers_session << endl;
        if (total_questions_session > 0) {
            cout << "점수: " << (int)(((double)correct_answers_session / total_questions_session) * 100) << "점!" << endl;
        } else {
            cout << "점수: 0점 (문제가 없었음)" << endl;
        }

        cout << "\n--- 학습 영역별 이해도 (이번 시험 기준) ---" << endl;
        ...
        if (player.hp > 0 && correct_answers_session == total_questions_session) {
            cout << "\n✨✨✨ 완벽해! " << player.name << "! 이번 쪽지 시험 모든 문제를 맞췄다냥! 대단하다냥! ✨✨✨" << endl;
            player.display("happy");
            pop_quiz_master.display("happy");
            continue_selection = false; // 100% 학습률 달성! 종료
        } else if (player.hp > 0) {
            cout << "\n" << player.name << ": 으음... 아직 부족한 부분이 있나? 100%가 될 때까지 다시 도전해야겠어!" << endl;
            player.display("thinking");
            cout << pop_quiz_master.name << ": 아직 완벽하지 않군! 다시 도전해서 실력을 완벽하게 만들겠다냥?" << endl;
            pop_quiz_master.display("default");
            ...
            if (retry_choice == 'y' || retry_choice == 'Y') {
                ...
                cout << "\n" << player.name << ": 좋아! 다시 한번 힘내보자! 이번엔 꼭 완벽하게 해낼 거야! 멍멍!" << endl;
                player.display("happy");
            } else {
            ...
            continue_selection = false; // 체력 0이 되면 바로 종료
        }
    } // end while (continue_selection)
}
...
int main() {
    ...
    // 모든 퀴즈 데이터 초기화
    vector<Question> pop_quiz_questions = initializePopQuizQuestions();
    
    // 카테고리 맵핑 (사용자에게 보여줄 번호와 카테고리 이름)
    std::map<int, string> categories_map;
    categories_map[1] = "Syntax";
    categories_map[2] = "Output";
    categories_map[3] = "Comments";
    categories_map[4] = "Variables";
    categories_map[5] = "Operators";
    categories_map[6] = "Data Types";
    categories_map[7] = "User Input";
    categories_map[8] = "Strings";
    categories_map[9] = "Math";
    categories_map[10] = "Booleans";
    categories_map[11] = "If-Else";
    categories_map[12] = "Switch";
    ...
    cout << "---------- C++ 쪽지 시험 시작! ----------" << endl;
    cout << "당신은 C++ 스터디의 귀여운 주인공, " << player_character.name << " (" << player_character.species << ") 입니다!" << endl;
    player_character.display("default");
    pressEnterToContinue(player_character, "happy", "지금까지 배운 내용을 점검해볼 시간이야!"); 

    // 쪽지 시험 시작
    conductPopQuiz(player_character, pop_quiz_questions, categories_map);

    cout << "\n\nC++ 쪽지 시험이 모두 끝났습니다. 정말 수고 많으셨습니다! 안녕!" << endl;
    player_character.display("default");
